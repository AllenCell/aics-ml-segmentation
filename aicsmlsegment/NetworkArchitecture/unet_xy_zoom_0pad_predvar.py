...
class UNet3D(nn.Module):
    def __init__(
        ...
        self.ec3 = self.encoder(
            128, 128, batchnorm=batchnorm_flag, padding=(1, 1, 1), dropout=self.dropout
        )  # 128 --> 256
        self.ec4 = self.encoder(
            256, 256, batchnorm=batchnorm_flag, padding=(1, 1, 1), dropout=self.dropout
        )  # 256 -->512
        ...
        self.dc3 = self.decoder(
            256 + 512, 256, batchnorm=batchnorm_flag, padding=(1, 1, 1), dropout=self.dropout
        )
        self.dc2 = self.decoder(
            128 + 256, 128, batchnorm=batchnorm_flag, padding=(1, 1, 1), dropout=self.dropout
        )
        ...
        # predict the model variance
        self.predict3 = nn.Conv3d(64, 1, 1)
    ...
    def forward(self, x):
        ...
        d0 = self.up0(u0)
        d0 = self.dc0(d0)
        predict00 = self.predict0(d0)
        # use a new branch to predict the model uncertainty, refer here:https://github.com/tanyanair/segmentation_uncertainty/blob/master/bunet/models/bunet.py
        ...
        return [predict00, predict03, p1a, p2a]
