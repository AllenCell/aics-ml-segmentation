...
class UNet3D(nn.Module):
    def __init__(self, in_channel, n_classes, down_ratio, batchnorm_flag=True):
        ...
        self.up0 = nn.ConvTranspose3d(64, 64, kernel_size=(1,k,k), stride=(1,k,k), padding=0, output_padding=0, bias=True)
        ...
        self.up1a = nn.ConvTranspose3d(128, n_classes[1], kernel_size=(1,2*k,2*k), stride=(1,2*k,2*k), padding=0, output_padding=0, bias=True)
        self.up2a = nn.ConvTranspose3d(256, n_classes[2], kernel_size=(1,4*k,4*k), stride=(1,4*k,4*k), padding=0, output_padding=0, bias=True)
    ...
    def forward(self, x):
        ...
        p0_final = predict00.permute(0, 2, 3, 4, 1).contiguous() # move the class channel to the last dimension
        p0_final = p0_final.view(p0_final.numel() // self.numClass, self.numClass)
        p0_final = self.softmax(p0_final, dim=1)

        p1a = F.pad(self.predict1a(self.conv1a(self.up1a(u1))),(-2*k-1,-2*k-1,-2*k-1,-2*k-1, -3, -3))
        ...
        p2a = F.pad(self.predict2a(self.conv2a(self.up2a(u2))),(-6*k-1,-6*k-1,-6*k-1,-6*k-1,-5,-5)) ## fix +5
